...
export const getMovementCredentials = async (params?: {
    ...
    const response = await apiClient.get(`/movement-credentials?${queryParams.toString()}`);
    return response.data;
};

// Buscar movimentação específica por ID
export const getMovementCredential = async (id: string): Promise<MovementCredentialResponse> => {
    const response = await apiClient.get(`/movement-credentials/${id}`);
    return response.data;
};

// Criar nova movimentação de credencial
export const createMovementCredential = async (data: CreateMovementCredentialRequest): Promise<MovementCredentialResponse> => {
    const response = await apiClient.post('/movement-credentials', data);
    return response.data;
};

// Atualizar movimentação de credencial
export const updateMovementCredential = async (id: string, data: UpdateMovementCredentialRequest): Promise<MovementCredentialResponse> => {
    const response = await apiClient.put(`/movement-credentials/${id}`, data);
    return response.data;
};

// Deletar movimentação de credencial
export const deleteMovementCredential = async (id: string): Promise<{ message: string }> => {
    const response = await apiClient.delete(`/movement-credentials/${id}`);
    return response.data;
};

// Buscar movimentação por participante e evento
export const getMovementCredentialByParticipant = async (eventId: string, participantId: string): Promise<MovementCredentialResponse | null> => {
    try {
...
export const changeCredentialCode = async (eventId: string, participantId: string, newCode: string, credentialsId?: string): Promise<MovementCredentialResponse> => {
