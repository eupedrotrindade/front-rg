...
export const getEmpresas = async (params?: PaginationParams): Promise<Empresa[] | null> => {
  try {
...
export const getEmpresasByEvent = async (eventId: string): Promise<Empresa[] | null> => {
  try {
    console.log("🔍 Buscando empresas do evento:", eventId);
    const { data } = await apiClient.get<{ data: Empresa[] }>(`/empresas/event/${eventId}`);
