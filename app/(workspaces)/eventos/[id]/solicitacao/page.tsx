...
import { useAllImportRequests } from '@/features/eventos/api/query/use-import-requests'
import { useApproveImportRequest } from '@/features/eventos/api/mutation/use-approve-import-request'
import { useRejectImportRequest } from '@/features/eventos/api/mutation/use-reject-import-request'
import type { ImportRequest } from '@/features/eventos/types'
...
const ImportRequestsDashboard = () => {
    ...
    const { data: evento = [], isLoading: eventoLoading } = useEventos({ id: String(params.id) })
    ...
    return (
        <EventLayout eventId={String(params.id)} eventName={(evento as Event)?.name || ''}>
                        ...
                        {selectedRequest && (
                            <div className="space-y-6">
                                    ...
                                    <div>
                                        ...
                                        <div className="grid grid-cols-2 gap-4">
                                            {selectedRequest.missingCredentials?.length > 0 && (
                                                <div>
                                                    <h5 className="text-sm font-medium text-gray-700 mb-2">Credenciais ({selectedRequest.missingCredentials?.length || 0})</h5>
                                            ...
                                            {selectedRequest.missingCompanies?.length > 0 && (
                                                <div>
                                                    <h5 className="text-sm font-medium text-gray-700 mb-2">Empresas ({selectedRequest.missingCompanies?.length || 0})</h5>
                                                    <div className="space-y-1">
                                ...
                                {selectedRequest.errors?.length > 0 && (
                                    <div>
                                        <h4 className="font-medium text-gray-900 mb-4">Erros de Validação ({selectedRequest.errors?.length || 0})</h4>
                                ...
                                {selectedRequest.duplicates?.length > 0 && (
                                    <div>
                                        <h4 className="font-medium text-gray-900 mb-4">Duplicatas ({selectedRequest.duplicates?.length || 0})</h4>
                                        <div className="max-h-40 overflow-y-auto space-y-2">
