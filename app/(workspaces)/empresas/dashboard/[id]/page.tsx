...
export default function EmpresaDashboardPage() {
    ...
    const { data: staff = [] } = useEventStaff()

    // Buscar eventos onde a empresa participou (baseado em participantes)
    const eventosParticipados = eventos.filter(evento => {
        const participantesDoEvento = participantes.filter(p => p.eventId === evento.id)
        return participantesDoEvento.some(p => p.company === empresa?.nome)
    })

    // Buscar participantes da empresa
    const participantesDaEmpresa = participantes.filter(p => p.company === empresa?.nome)

    // Buscar staff da empresa
    const staffDaEmpresa = staff.filter(s => s.company === empresa?.nome)
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        {staffDaEmpresa.map((staffMember) => {
                                            const eventosDoStaff = eventos.filter(e =>
                                                staffDaEmpresa.some(s => s.eventId === e.id && s.id === staffMember.id)
                                            )
